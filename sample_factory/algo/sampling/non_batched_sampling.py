...
class ActorState:
    ...
    def record_env_step(self, reward, terminated: bool, truncated: bool, info, rollout_step):
        ...
        self.curr_traj_buffer["costs"][rollout_step] = float(info.get("cost", 0.0))
    ...
    def _episodic_stats(self, info: Dict) -> Dict[str, Any]:
        stats = dict(
            ...
            heatmap=info.get("previous_histogram", []),
        )
...
class NonBatchedVectorEnvRunner(VectorEnvRunner):
    ...
    def init(self, timing: Timing):
        ...
        for env_i in range(self.num_envs):
            ...
            env = make_env_func_non_batched(self.cfg, env_config=env_config, render_mode=self.cfg.render_mode)
