...
class SAC:
    ...
    def run(self):
        ...
        action_counts = {i: 0 for i in range(num_actions)}
        reasoning = ''
        scenario = self.env.get_active_env().scenario

        for global_timestep in range(self.steps):
            action, reasoning = self.get_gpt_action(reasoning, obs, scenario)
    ...
    def get_gpt_action(self, reasoning, obs, scenario):
        if scenario == 'health_gathering':
            prompt = f"The agent is tasked to collect health kits in the environment and not running out of health " \
                     ...
                     f"To further aid you in this task, this is the justification you provided at the previous time step: {reasoning}"
        elif scenario == 'arms_dealer':
