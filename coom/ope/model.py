...
class MDN_RNN(chainer.Chain):
    ...
    def __call__(self, z_t, action, temperature=1.0, cpu=True):
        k = self.k
...
class TBPTTUpdater(training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        total_loss = 0
        z_t, z_t_plus_1, action, reward, done = self.converter(batch, self.device)
...
def main():
    ...
    sample_size = 256
    rollout_z_t, rollout_z_t_plus_1, rollout_action, reward, done = train[0]
    ...
    rollout_z_t, rollout_z_t_plus_1, rollout_action, reward, done = train[
